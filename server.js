...
whatsappClient.on("qr", async (qr) => {
    try {
        latestWhatsAppQR = await QRCode.toDataURL(qr);

        console.log("");
        console.log("==============================================");
        console.log("SCAN WHATSAPP QR CODE");
        console.log("Open /api/whatsapp-qr");
        console.log("WhatsApp > Linked Devices > Link a Device");
        console.log("==============================================");
...
app.post("/api/send-surveys-batch", async (req, res) => {
    try {
        if (!whatsappReady) {
        ...
        for (const trainee of result.rows) {
            try {
                let cleanPhone = String(trainee.phone).replace(/\D/g, "");
                ...
                const message =
`Namaskar ${trainee.name || "Trainee"},

Greetings from KaushalSetu Maharashtra Skill Mission!

We are updating your training outcome record.
...
Your response helps us improve skill-training outcomes.
...
KaushalSetu`;
                ...
                console.log(
                    `[WhatsApp Batch] Survey sent to ${cleanPhone}`
                );
                ...
                console.error(
                    `[WhatsApp Batch] Failed for ${trainee.phone}:`,
                    error.message
                );
            }
        }
        ...
        console.error("Batch survey error:", error);
